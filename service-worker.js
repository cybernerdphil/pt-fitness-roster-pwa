/**
 * SERVICE WORKER — PT & FITNESS ROSTER AVAILABILITY FORM
...
const CACHE_STATIC   = `static-v${VERSION}-pt-fitness`;
const CACHE_FONTS    = `font-v${VERSION}-pt-fitness`;
const CACHE_EXTERNAL = `external-v${VERSION}-pt-fitness`;
...
self.addEventListener('install', (event) => {
  console.log(`[SW] Installing PT & Fitness service worker v${VERSION}`);
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating PT & Fitness service worker');

  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (!cacheName.includes(`v${VERSION}-pt-fitness`)) {
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const title = data.title || 'PT & Fitness Roster';
    const options = {
      body: data.body || 'You have a new roster message from your facility manager',
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) {
        if (client.url.includes('/pt-fitness-roster-pwa/') && 'focus' in client) {
          return client.focus();
        }
      }
...
console.log(`[SW] PT & Fitness service worker v${VERSION} loaded`);
