(() => {
  ...
  function buildAvailabilityTable() {
    ...
    DAYS.forEach(day => {
      ...
      // Start cell — default 05:30 (early gym opens)
      const startCell  = document.createElement('td');
      const startRow   = document.createElement('div');
      startRow.className = 'sc-time-cell';
      startRow.id = 'sc-srow-' + day.key;
      startRow.appendChild(buildTimeSelect('sc-start-' + day.key, '05:30'));
      ...
      // End cell — default 21:00 (late classes end)
      ...
      endRow.appendChild(buildTimeSelect('sc-end-' + day.key, '21:00'));
  ...
  function buildPayload() {
    ...
    return {
      ...
      cert:           sanitize($('sc-cert').value, 80),
      specialty:      sanitize($('sc-specialty').value, 80),
      availability:   av
    };
  }
  ...
  function makeFilename(name) {
    const slug = (name || 'trainer')
      .replace(/\s+/g, '-').toLowerCase().replace(/[^a-z0-9-]/g, '');
    return 'pt-avail-' + slug + '-' + new Date().toISOString().slice(0, 10) + '.json';
  }
  ...
  function openEmail(managerEmail, memberName, filename) {
    const subj = encodeURIComponent('PT & Fitness Roster Availability — ' + memberName);
    const body = encodeURIComponent(
      'Hi,\n\nPlease find my weekly availability submission attached as:\n    ' + filename +
      '\n\nTo import it into the PT & Fitness Roster, go to:\n    Backup → Import Availability → select the attached file\n\n' +
      'Submitted by: '  + memberName + '\nSubmitted at: ' + new Date().toLocaleString() +
      '\n\n— Sent via PT & Fitness Roster Availability Form'
    );
  ...
  async function handleShare() {
    ...
    try {
      await navigator.share({
        title: 'PT & Fitness Roster Availability',
        text:  'Weekly availability for ' + (payload.memberName || 'trainer'),
        files: [file]
      });
